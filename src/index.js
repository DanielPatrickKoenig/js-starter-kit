...
function mapToShape (index) {
  plotter.setPoints(shapes[index].data, .5);
  for(let i = 1; i < particles.length; i++){
      const targetPos = plotter.plot(i/particles.length);
      ...
      tweenSteps(particles[i], [{duration: xInDuration, ease: Math.random() > .5 ? Sine.easeIn : Sine.easeOut, values: {x: getRandomPointPosition().x}}, {duration: xOutDuration, ease: Math.random() > .5 ? Sine.easeOut : Sine.easeIn, values: {x: targetPos.x}}], update, completed);
      tweenSteps(particles[i], [{duration: yInDuration, ease: Math.random() > .5 ? Sine.easeOut : Sine.easeIn, values: {y: getRandomPointPosition().y}}, {duration: yOutDuration, ease: Math.random() > .5 ? Sine.easeIn : Sine.easeOut, values: {y: targetPos.y}}], update, completed);
  }
}
function update(){
...
function init (count) {
  ...
  plotter = new ShapePlotter(shapes[0].data);
